            ...
            <div className="report-content">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4 mb-8">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-green-500">
                        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase">Total Sales</p>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mt-1">{formatPrice(salesData.totalSales)}</h2>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Excludes cancelled orders</p>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-blue-500">
                        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase">Order Count</p>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mt-1">{salesData.orderCount}</h2>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Total orders in this period</p>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-purple-500">
                        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase">Avg. Order Value</p>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mt-1">{formatPrice(salesData.avgOrderValue)}</h2>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Revenue per order</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-6">Payment Method Breakdown</h3>
                    ...
                    {/* Top Customers */}
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-6">Top Performing Customers</h3>
                        <div className="space-y-4">
                            {customerData.topCustomers.map((customer: any) => (
                                <div key={customer.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-primary/10 dark:bg-primary/20 rounded-full flex items-center justify-center text-primary dark:text-primary-400 text-sm font-bold">
                                            {customer.name.split(' ').map((n: string) => n[0]).join('')}
                                        </div>
                                        <div>
                                            <p className="font-bold text-gray-800 dark:text-white text-sm">{customer.name}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{customer.orderCount} orders</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-primary dark:text-primary-400">{formatPrice(customer.totalSpent)}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-widest">Lifetime Value</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl border-2 border-primary/10 dark:border-primary/5 shadow-lg flex flex-col justify-center">
                        <div className="flex items-end justify-between">
                            <div>
                                <p className="text-gray-500 dark:text-gray-400 text-sm font-bold uppercase tracking-wider">Conversion Ratio</p>
                                <h2 className="text-4xl font-black text-gray-800 dark:text-white mt-2">
                                    {((customerData.activeCustomers / customerData.totalCustomers) * 100).toFixed(1)}%
                                </h2>
                                <p className="text-gray-400 dark:text-gray-500 text-xs mt-2 italic">Active customers with delivered orders</p>
                            </div>
                            <div className="text-right">
                                <span className="text-primary dark:text-primary-400 font-black text-xl">{customerData.activeCustomers}</span>
                                <span className="text-gray-400 dark:text-gray-500 text-sm ml-1">Purchased</span>
                            </div>
                        </div>
                    </div>
                </div>
